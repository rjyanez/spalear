        ...
        <div class="px-4">
          <div class="row justify-content-center">
              ...
              <div class="card-profile-actions py-4 mt-lg-0 d-flex align-items-center">
                <div class="card-profile-actions pb-4 mt-lg-0 d-flex align-items-center">
                  <template v-if="isCurrent">
                    ...
                    <router-link
                      class="mr-4 float-right btn btn-sm btn-primary"
                      to="teacher/favorite"
                    >
                  ...
                  <template v-if="!isCurrent && isRole('TE')">
                    ...
                    />
                    <button-message
                      @submitMessage="searchStudent" 
                      action="/api/student/message" 
                      :to="student.id"
                      class="mr-4"
                    />
                  </template>
            ...
            <div class="col-lg-4 order-lg-1">
              <div
                v-if="student.hasOwnProperty('progress')"
          ...
          <div class="text-center mt-3 pb-4 border-bottom">
            ...
            <div class="h4 font-weight-300">
              <i class="ni location_pin mr-2">{{ student. timeZone}} - {{ student.country }}</i>
            </div>
            <div>
              <i class="ni education_hat mr-2">{{ student.description }}</i>
            </div>
          </div>
          <div class="py-5 border-top text-center">
            <meetings-list
              @searchUser="searchStudent"
              title="Next Classes"
...
export default {
  ...
  data() {
    return {
      student: {
        id:
          parseInt(this.$route.params.id) || this.$store.getters.currentUser.id
      }
    };
  },
  computed: {
  ...
  methods: {
    ...
    redirectTeachers() {
      this.$toasted.info(
        "To schedule a class you must first select a teacher."
      );
    ...
    searchStudent() {
      this.$store
        .dispatch("sendGet", {
          url: `/api/student/${this.student.id}`,
          auth: true
        })
        .then(res => {
